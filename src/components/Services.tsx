...
export const Services: React.FC<ServicesProps> = () => {
  ...
  // State to track selected option (duration & price) per treatment card
  const [selectedOptions, setSelectedOptions] = useState<Record<string, { durationMinutes: number; price: number }>>({});

  // Helper to get active pricing/duration for a treatment
  const getActivePricing = (treatment: Treatment) => {
    return (
      selectedOptions[treatment.id] || {
  ...
  // Helper to handle option changes (duration selection)
  const handleOptionChange = (treatmentId: string, durationMinutes: number, price: number) => {
    setSelectedOptions((prev) => ({
      ...prev,
      [treatmentId]: { durationMinutes, price },
    }));
  };

  // Helper to open WhatsApp directly with pre-filled message
  const handleWhatsAppBooking = (treatment: Treatment) => {
    const active = getActivePricing(treatment);
    const phoneNumber = "1234567890"; // Replace with your target WhatsApp phone number
    const message = encodeURIComponent(
      `Hello, I would like to book the ${treatment.name} treatment (${active.durationMinutes} mins - $${active.price}).`
    );
  ...
  return (
    <section id="services" className="py-12 lg:py-20 bg-white relative">
      <div className="max-w-7xl mx-auto px-6 sm:px-8">
        ...
        {/* Section Header */}
        <div className="max-w-2xl space-y-4 mb-12 text-center sm:text-left">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#F2ECE4] border border-[#C6A473]/30 text-[#A37B57] text-xs font-jakarta font-medium tracking-wide">
            <Sparkles className="w-3.5 h-3.5" />
            <span>Bespoke treatment menu</span>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {TREATMENTS.map((treatment) => {
            const activePricing = getActivePricing(treatment);
            const hasMultipleOptions = treatment.options && treatment.options.length > 0;

            // Gather all available options into a unified array
            const allOptions = [
              { durationMinutes: treatment.durationMinutes, price: treatment.price },
              ...(treatment.options || []),
            ];
            ...
            return (
              <div
                key={treatment.id}
                className="bg-white rounded-3xl overflow-hidden border border-[#F2ECE4] shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between group"
              >
                <div>
                  {/* Image & Badges */}
                  <div className="relative h-60 w-full overflow-hidden bg-[#24201C]">
                    <img
                      src={treatment.image}
                      alt={treatment.name}
                      referrerPolicy="no-referrer"
                    ...
                    {/* Category Tag */}
                    {/* <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-jakarta font-semibold uppercase tracking-wider text-[#C6A473] border border-white/10">
                      {treatment.category}
                    </div> */}

                    {/* Dynamic Price Badge */}
                    <div className="absolute bottom-4 right-4 bg-white/95 backdrop-blur-md px-3 py-1 rounded-full text-sm font-jakarta font-bold text-[#222222] shadow-sm">
                      {hasMultipleOptions && !selectedOptions[treatment.id] ? `From $${treatment.price}` : `$${activePricing.price}`}
                    </div>
                  </div>

                  {/* Card Content */}
                  <div className="p-6 space-y-3">
                    <div className="flex items-center gap-1.5 text-xs font-jakarta text-[#A37B57] font-medium">
                      <Clock className="w-3.5 h-3.5" />
                      <span>{activePricing.durationMinutes} minutes</span>
                    </div>

                    <h3 className="font-playfair text-2xl text-[#222222] font-medium leading-tight">
                      {treatment.name}
                    </h3>

                    <p className="font-inter text-xs sm:text-sm text-[#666666] leading-relaxed line-clamp-3">
                      {treatment.description}
                    </p>
                    ...
                    {hasMultipleOptions && (
                      <div className="pt-2 flex items-center gap-2">
                        <span className="text-[11px] font-jakarta text-[#888888]">Duration:</span>
                        <div className="flex items-center gap-1.5">
                          {allOptions.map((opt) => {
                            const isSelected = activePricing.durationMinutes === opt.durationMinutes;
                            return (
                              <button
                                key={opt.durationMinutes}
                                onClick={() => handleOptionChange(treatment.id, opt.durationMinutes, opt.price)}
                                className={`px-2.5 py-1 rounded-full text-xs font-jakarta transition-all ${
                                  isSelected
                                    ? 'bg-[#A37B57] text-white font-semibold'
                                    : 'bg-[#F2ECE4]/60 text-[#666666] hover:bg-[#F2ECE4]'
                                }`}
                              >
                ...
                {/* Card Footer Actions */}
                <div className="px-6 pb-6 pt-2 flex items-center justify-between gap-3 border-t border-[#F2ECE4]/60 mt-auto">
                  <button
                    onClick={() => setPreviewTreatment(treatment)}
                    className="text-xs font-jakarta font-medium text-[#666666] hover:text-[#222222] underline underline-offset-4"
                  >
                    Quick view
                  </button>

                  <a href="https://wa.me/971554690739" target="_blank" 
                    onClick={() => handleWhatsAppBooking(treatment)}
                    className="flex items-center gap-2 bg-[#25D366] hover:bg-[#20bd5a] text-white text-xs font-jakarta font-semibold px-4 py-2.5 rounded-full transition-all duration-300 shadow-sm hover:shadow-md"
                  >
      ...
      {/* Quick View Modal with Multi-pricing support */}
      {previewTreatment && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 space-y-6 shadow-2xl border border-[#F2ECE4] relative max-h-[90vh] overflow-y-auto">
            <button
              onClick={() => setPreviewTreatment(null)}
              className="absolute top-2 right-2 w-8 h-8 rounded-full bg-[#FAF8F5] text-[#666666] hover:text-[#222222] flex items-center justify-center font-bold"
            >
              ✕
            </button>
            ...
            <div className="relative h-64 rounded-2xl overflow-hidden">
              ...
              />
              <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-jakarta font-bold text-[#222222]">
                ${getActivePricing(previewTreatment).price} • {getActivePricing(previewTreatment).durationMinutes} minutes
              </div>
            </div>
            ...
            {/* Duration choices inside modal */}
            {previewTreatment.options && previewTreatment.options.length > 0 && (
              <div className="space-y-2">
                <span className="text-xs font-jakarta text-[#888888] font-medium block">
                  Select Duration:
                </span>
                <div className="flex items-center gap-2">
                  {[
                    { durationMinutes: previewTreatment.durationMinutes, price: previewTreatment.price },
                    ...previewTreatment.options,
                  ].map((opt) => {
                    const isSelected = getActivePricing(previewTreatment).durationMinutes === opt.durationMinutes;
                    return (
                      <button
                        key={opt.durationMinutes}
                        onClick={() => handleOptionChange(previewTreatment.id, opt.durationMinutes, opt.price)}
                        className={`px-3.5 py-1.5 rounded-full text-xs font-jakarta transition-all ${
                          isSelected
                            ? 'bg-[#A37B57] text-white font-semibold'
                            : 'bg-[#F2ECE4]/60 text-[#666666] hover:bg-[#F2ECE4]'
                        }`}
                      >
                        {opt.durationMinutes} Minutes - ${opt.price}
                      </button>
                    );
                  })}
                </div>
            ...
            <div className="pt-4 border-t border-[#F2ECE4] flex items-center justify-between">
              <div className="invisible text-xs text-[#666666] font-inter">
                Includes private shower suite & organic tea ceremony.
              </div>
              <a href="https://wa.me/971554690739" target="_blank" 
                onClick={() => {
                  const selected = previewTreatment;
                  setPreviewTreatment(null);
                  handleWhatsAppBooking(selected);
                }}
                className="flex items-center gap-2 bg-[#25D366] hover:bg-[#20bd5a] text-white font-jakarta text-xs sm:text-sm font-semibold px-6 py-3 rounded-full transition-all shadow-md"
              >
                <span>Book Now</span>
                    <FaWhatsapp className="w-4 h-4 fill-current" />
              </a>
